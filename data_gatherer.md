Yes. Use this **single-file data gatherer** first. It works immediately with:

* seed URLs you define
* free GDELT news search, no API key
* RSS feeds
* local PDFs you drop into `data/raw/pdf/`
* outputs `document_index.csv` and `paragraph_level_dataset.csv`
...
pip install requests beautifulsoup4 pandas trafilatura feedparser pdfplumber tqdm python-dateutil
```

---

## 2. Create `data_gatherer.py`
...
SEED_URLS_PATH = SOURCES_DIR / "seed_urls.csv"
RSS_FEEDS_PATH = SOURCES_DIR / "rss_feeds.csv"

USER_AGENT = "Mozilla/5.0 research-bot/1.0 for student equity research"
...
def split_into_paragraphs(text: str, min_chars: int = 120, max_chars: int = 1800) -> list[str]:
...
```bash
python data_gatherer.py
```

It will automatically create:

```text
sources/seed_urls.csv
sources/rss_feeds.csv
data/raw/text/
data/raw/pdf/
...
Open the paragraph dataset:

```bash
python - <<'PY'
import pandas as pd

df = pd.read_csv("data/processed/paragraph_level_dataset.csv")
print(df.head())
print("\nParagraphs:", len(df))
print("\nThemes:")
...
## 5. Add your own serious sources
...
Sieyuan Annual Report,PASTE_URL_HERE,Sieyuan Electric,Grid Infrastructure,Annual Report,Grid Resilience
Halliburton Q1 Transcript,PASTE_URL_HERE,Halliburton,Oilfield Services,Transcript,Oilfield Cost Pressure
SLB Q1 Transcript,PASTE_URL_HERE,SLB,Oilfield Services,Transcript,Oilfield Cost Pressure
```

For PDFs, just drop them into:
...
This gives you the working raw dataset for your AI classifier.
